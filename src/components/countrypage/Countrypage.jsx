...
export default function Countrypage() {
  ...
  useEffect(() => {
    fetch("https://restcountries.com/v3.1/all?fields=name,capital,region,population,flags,cca3,borders,currencies,languages,tld")
      .then((response) => {
        if (!response.ok) {
  ...
  const getBorderCountries = () => {
    if (country && country.borders && allCountries.length > 0) {
      const borderCountries = country.borders.map((borderCode) => {
        // Find the full country name from the border code
        const borderCountry = allCountries.find((c) => c.cca3 === borderCode);
        const countryName = borderCountry ? borderCountry.name.common : borderCode;
